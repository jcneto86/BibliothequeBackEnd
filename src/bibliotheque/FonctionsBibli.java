...
public class FonctionsBibli {
	...
	public static ArrayList<Livre> rechercheLivre(String titreLivreOuNomAuteur) {
		...
		try {
			...
			while ( rs.next() ) {
				...
				Livre livre = new Livre( titre,  nomauteur,  annee,  edition,  nb_pages,  maison_edition,
						corridor,  cote,  rangee,  rayon,  position,  id_exemplaire);
				livres.add(livre);
	...
	public static void reserverLivre(ReservationLivre reservationLivre) {
		SQLClass.connect();
		try {
			ResultSet rs = SQLClass.executeQuery("INSERT INTO RESERVATION_LIVRE \n" + 
	...
	public static void fairePret(Pret pret, Utilisateur utilisateur, ArrayList<Exemplaire> exemplaires) {
		SQLClass.connect();
		try {
			ResultSet rs = SQLClass.executeQuery("INSERT INTO PRET VALUES \n"
					+ "(NULL, ID_UTILISATEUR, '2018-03-05', DATE_ADD(DATE_RETIREE, INTERVAL 15 DAY), 0);\n" + 
					""		
					);
			while ( rs.next() ) {
			}
	...
	public static void enregistrerUtilisateur(Utilisateur utilisateur, Adresse adresse) {
		...
		try {
			ResultSet rs = SQLClass.executeQuery("SELECT COUNT(*) as nbRegist, id_adresse  \n"
					+ "FROM adresse \n"
					+ "WHERE coordonner LIKE '" + adresse.getCoordonner() 
					+ "' AND nb LIKE '" + adresse.getNb() 
					+ "' AND codepostal LIKE '" 
			...
			while ( rs.next() ) {
				...
				if (nbRegist == 0) {
					//registrar
					SQLClass.executeQuery("INSERT INTO ADRESSE VALUES \n"
							+ "(NULL, '" + adresse.getCoordonner() 
							+ "', " + adresse.getNb() 
							+ ", '" + adresse.getCodepostal() 
					...
					SQLClass.executeQuery("INSERT INTO UTILISATEUR VALUES (NULL, , '"
							+ adresse.getId_adresse() +"', '" 
							+ utilisateur.getNom() + "', '" 
							+ utilisateur.getNom_de_famille() + "', '" 
							+ utilisateur.getEmail() + "' , AES_ENCRYPT('" + utilisateur.getPassword() + "', '"+ utilisateur.getPassword() + "'), '"
	...
	public static void enregistrerLivre(Auteur auteur, Edition edition, OevreLitteraire oevreLitteraire, MaisonEdition maisonEdition, LocalizationLivre localizationLivre) {
		...
		try {
			...
			while ( rs.next() ) {
				...
				if (nbRegist == 0) {
					//registrar
					SQLClass.executeQuery("INSERT INTO AUTEUR VALUES (NULL, '" + auteur.getNomAuteur() + "');");
			...
			while ( rs.next() ) {
				...
				if (nbRegist == 0) {
					//registrar
					SQLClass.executeQuery("INSERT INTO oeuvre_litteraire VALUES (NULL, "
							+ "'" + auteur.getId_auteur() 
							+ "', '"+ oevreLitteraire.getTitre() 
							+"', '"+ oevreLitteraire.getLangue_originale() 
							+ "', "+oevreLitteraire.getAnnee() + ");");
			...
			while ( rs.next() ) {
				...
				if (nbRegist == 0) {
					//registrar
					SQLClass.executeQuery("INSERT INTO maison_edition VALUES (NULL, "
							+ "'" + maisonEdition.getNom() +");");
			...
			while ( rs.next() ) {
				...
				if (nbRegist == 0) {
					//registrar
					SQLClass.executeQuery("INSERT INTO EDITION VALUES (NULL, "
							+ maisonEdition.getId_maison_edition() + ", "
							+ oevreLitteraire.getId_oeuvre_litteraire() + ", "
							+ edition.getAnnee() + ", "
							+ "'"+ edition.getEdition() + "', "
							+ edition.getNb_pages() +", "
							+ "'" + edition.getIsbn() + "', "
			...
			rs = SQLClass.executeQuery("SELECT COUNT(*) as nbRegist, id_localization_livre \n"
					+ "FROM localization_livre WHERE corridor LIKE '" + localizationLivre.getCorridor() 
					+ "' AND cote LIKE '" + localizationLivre.getCote() 
					+ "' AND rangee LIKE '"+ localizationLivre.getRangee() 
					+ "' AND rayon LIKE '" + localizationLivre.getRayon() 
					+ "' AND position LIKE '" + localizationLivre.getPosition() + "';");
			while ( rs.next() ) {
				int nbRegist = rs.getInt("nbRegist");
				if (nbRegist == 0) {
					//registrar
					SQLClass.executeQuery("INSERT INTO LOCALIZATION_LIVRE VALUES (NULL, '"+ localizationLivre.getCorridor() 
					+"', '"+ localizationLivre.getCote()  
					+"', '"+ localizationLivre.getRangee() 
					+"', '"+ localizationLivre.getRayon()  
					+"', '"+ localizationLivre.getPosition() +"');");
			...
			// gegistrar exemplar
			SQLClass.executeQuery("INSERT INTO EXEMPLAIRE VALUES \n"
